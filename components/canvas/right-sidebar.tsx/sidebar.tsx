...
import { PixabaySearch } from "@/components/pixabay/search_input";

interface RightToolbarProps {
  currentRatio: RatioKey;
  onBackgroundChange: (config: BackgroundConfig) => void;
  onRatioChange: (ratio: RatioKey) => void;
  setIsPixabayOpen: (flag: boolean) => void;
  isPixabayOpen: boolean;
  handlePixabaySelect: (imageUrl: string) => void; // ✅ Новый пропс
}
...
export function RightToolbar({
  ...
  return (
    <aside className="w-72 shrink-0 m-4 rounded-4xl border overflow-y-auto bg-muted/50 backdrop-blur flex flex-col overflow-hidden text-sm select-none">
        ...
        <div className="space-y-4">
          <div className="flex flex-col space-y-4">
            <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
              ...
              <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded text-foreground/70">
                {currentRatio}
              </span>
            </div>
            ...
            <div className="space-y-2 mt-2">
              <div className="flex items-center justify-between bg-muted/30 px-3.5 py-2 rounded-xl">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs font-mono text-zinc-200 w-6 text-right">
                    {opacity}
                  </span>
                  <Slider
                    value={[opacity]}
